...
function [Z,M,H,moments_edge_removed] = kacward_momentsedgeremoved(points,edges,K,require_hessian)
...
M = zeros(1,m);
H = zeros(m);
moments_edge_removed = zeros(1,m);

if( require_hessian )
    for index = 1 : m
        M(index) = real( w(index) - 0.5* ( (1-w(index)^2) * ( U(index,index)+U(m+index,m+index) ) ) );
        moments_edge_removed(index) = (M(index) - w(index)) / (1 - (M(index)*w(index)));
        for index_inner = 1 : m
